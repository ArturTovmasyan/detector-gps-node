...
function getDataGroupedByIMEIs(from, to, callback) {
    ...
    if (from && to) {
        ...
        d = "d.timestamp >= '" + from + "' AND d.timestamp <= '" + to + "'";
        where = "WHERE ";
        and = "AND";
    }
    ...
    return connection.query(
        ...
        "SELECT okData.imei, allData.allCount, COALESCE(okData.okCount, 0) as okCount " +
        "FROM " +
        "(SELECT COUNT(d.id) as allCount, d.imei " +
        "FROM gps_info as d " + where + d + " GROUP BY d.imei) allData " +
        "LEFT JOIN " +
        "(SELECT COUNT(d.id) as okCount, d.imei " +
        "FROM gps_info as d WHERE " + d + and + " d.satellites != 0 GROUP BY d.imei) okData " +
        "ON okData.imei = allData.imei ",
        callback
    );
}
...
//This function is used to save incoming data from gps
