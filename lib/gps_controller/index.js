...
function AVLDataParse(data, offset) {
    ...
    var parsedData = {};
    var timestamp = data.readUInt32BE(offset) * 0x100000000 + data.readUInt32BE(offset + 4);
    ...
    log.debug('timestamp: '  + busInfo.data[timestamp].timestamp);
    log.debug('priority: '   + busInfo.data[timestamp].priority);
    log.debug('longitude: '  + busInfo.data[timestamp].longitude);
    log.debug('latitude: '   + busInfo.data[timestamp].latitude);
    log.debug('altitude: '   + busInfo.data[timestamp].altitude);
    log.debug('angle: '      + busInfo.data[timestamp].angle);
    log.debug('satellites: ' + busInfo.data[timestamp].satellites);
    log.debug('speed: '      + busInfo.data[timestamp].speed);
