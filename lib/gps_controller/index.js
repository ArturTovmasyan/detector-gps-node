        ...
        socket.on('data', function (data) {
            ...
            if (data.readInt16BE(0) === 15) {
                ...
                for (var i = 0; i < NumberOfData; i++) {
                    ...
                    console.log('timestamp: ' + busInfo.data[timestamp].timestamp);
                    console.log('priority: ' + busInfo.data[timestamp].priority);
                    console.log('longitude: ' + busInfo.data[timestamp].longitude);
                    console.log('latitude: ' + busInfo.data[timestamp].latitude);
                    console.log('altitude: ' + busInfo.data[timestamp].altitude);
                    console.log('angle: ' + busInfo.data[timestamp].angle);
                    console.log('satellites: ' + busInfo.data[timestamp].satellites);
                    console.log('speed: ' + busInfo.data[timestamp].speed);

                    //Go throw IO Elements
                    curIndex += 24 + 2;
                    curIndex += data.readUInt8(curIndex) + 1;
                    console.log('first end: ' + curIndex);
                    curIndex += data.readUInt8(curIndex) + 1;
                    console.log('second end: ' + curIndex);
                    curIndex += data.readUInt8(curIndex) + 1;
                    console.log('third end: ' + curIndex);
                    curIndex += data.readUInt8(curIndex) + 1;
                    console.log('fourth end: ' + curIndex);

                    //Save data to database
                    db.flush(busInfo.IMEI, busInfo.data[timestamp], function(err) {
