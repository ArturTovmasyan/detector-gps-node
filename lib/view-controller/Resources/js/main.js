...
$(document).ready(function(){
    ...
    var lastFrontMarker = null;
    var lastBackMarker  = null;
            ...
            for (var k in msgs) {
                ...
                if (!markers[gpsData.imei]) {
                    var marker = new google.maps.Marker({
                        ...
                        title: 'imei:' + gpsData.imei + '\nspeed:' + gpsData.speed + '\nplateNumber:' + msg.busInfo.plateNumber + '\nrouteId:' + routeId + '\ntoNearestBus:' + toNearestBus + '\ndataTime: ' + $.formatDateTime('mm/dd/y h:i:s', new Date(gpsData.timestamp)) + '\nangle: ' + msg.busInfo.angle,
                        icon: busIcon
                    });
                    ...
                    google.maps.event.addListener(marker, 'click', function() {
                        ...
                        if(lastFrontMarker){
                            markers[lastFrontMarker].setAnimation(null);
                        }
                        if(lastBackMarker){
                            markers[lastBackMarker].setAnimation(null);
                        }

                        if (markers[this.frontImei]) {
                            markers[this.frontImei].setAnimation(google.maps.Animation.BOUNCE);
                        }
                        if (markers[this.backImei]) {
                            markers[this.backImei].setAnimation(google.maps.Animation.BOUNCE);
                        }

                        lastFrontMarker = this.frontImei;
                        lastBackMarker  = this.backImei;
                    });
                    ...
                    markers[gpsData.imei].frontImei = msg.busInfo.frontImei;
                    markers[gpsData.imei].backImei  = msg.busInfo.backImei;
    ...
    function filterBuses(){
        ...
        for(var key in markers){
            if(choosenLines.indexOf(markers[key].lineNumber) === -1){
                ...
                if(bus_section_part_marker[key]) {
                    bus_section_part_marker[key].setMap(null);
                }
            }
            else {
                if(!markers[key].getMap()) {
                    markers[key].setMap(map);
                }
                if(bus_section_part_marker[key] && !bus_section_part_marker[key].getMap()) {
    ...
    $.getJSON('http://80.86.238.52:81/api/skeletons', function (sections) {
        ...
        for(var key in polylines){
            $.getJSON('http://80.86.238.52:81/api/lines/' + key + '/section/parts', function (sp) {
