...
$(document).ready(function(){
    ...
    socket
        ...
        .on('message',function(msgs){
            ...
            for (var k in msgs) {
                ...
                updateLinesList(lineNumber, gpsData.imei);
    ...
    function filterBuses(){
        if(jQuery.isEmptyObject(linesImeis)){
            return;
        }

        for(var imei in markers){
            for(var line in linesImeis){
                if(linesImeis[line].indexOf(key) === -1){
    ...
    function updateLinesList(lineN, imei){
        if(!linesImeis[lineN]){
            linesImeis[lineN] = [];
            var template = '<label class="line-list">'+
                '<input type="checkbox" style="float: onchange="chooseLine()" left" value="'+lineN+'"/>'+
                '<span style="float: left;margin-left: 10px"> '+lineN+'</span>'+
            '</label>';

            $("#line-list").append(template);
        }
        if(linesImeis[lineN].indexOf(imei) !== -1) {
            linesImeis[lineN].push(imei);
        }
