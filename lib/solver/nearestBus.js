...
function determineNearestBus(imei, routeId, sectionId){
    ...
    //Load sections of line_number line
    var sections     = loader.findSectionsByLine(line_number);

    //Create new field for line_number Line
    if (!busPositionsData[line_number]){
        busPositionsData[line_number] = {};
    }

    //If there aren't determined route remove data of given imei
    if (!routeId){
        sections[sectionId].route_section.forEach(function (routeSection){
            if (busPositionsData[line_number][routeSection.route.id] && busPositionsData[line_number][routeSection.route.id][imei]) {

                //Change front and back buses front and back buses :)
                var frontImei = busPositionsData[line_number][routeSection.route.id][imei].frontImei;
                var backImei  = busPositionsData[line_number][routeSection.route.id][imei].backImei;

                busPositionsData[line_number][routeSection.route.id][frontImei].backImei = backImei;
                busPositionsData[line_number][routeSection.route.id][backImei].frontImei = frontImei;

                //Remove data of imei
                delete busPositionsData[line_number][routeSection.route.id][imei];
            }
        });
    ...
    if (!busPositionsData[line_number][routeId]){
