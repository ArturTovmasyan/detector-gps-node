...
function determineSectionOrderAngle(imei, routeId, sectionId)
{
    ...
    for (var key in sections[sectionId].route_section){
        ...
        if (routeSection.route.id == routeId){
            var angle = (routeSection.direction > 0) ? sections[sectionId].angle : (180 + sections[sectionId].angle);
            cache.put('determineSectionOrder_' + imei, {"route_id": routeId, "order": routeSection.order, "angle": angle}, cache_conf.expiration_time);
            return {"order": routeSection.order, "angle": angle};
        }
    }
