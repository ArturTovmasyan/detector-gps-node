...
function findNearestSectionPart(item){
    if(!item.imei || !item.latitude || !item.longitude){
        ...
        return new Error("The item is not valid");
    }

    var line_number = loader.lines[item.imei];
    //If lines didn't loaded then return error with corresponding message
    if (!line_number) {
        return new Error("Lines yet didn't loaded. You can try after few minutes");
    }

    var section_parts = loader.loadLineSectionParts[line_number];
    //If line's section parts didn't loaded then start to load and return error with corresponding message
    if (!section_parts) {
        loader.loadLineSectionParts(line_number);
        return new Error("Section parts yet didn't loaded. You can try after few minutes");
    }

    var item_distance = distanceFromStart(item.latitude, item.longitude);
    var current_index = Math.floor(section_parts.length / 2);

    var tmp_difference = item_distance - section_parts[current_index].distance;
    var dif_index = current_index;
    var length = section_parts.length;

    while(length) {
        if (item_distance > section_parts[current_index].distance) {
            length = Math.floor(section_parts.length / 2);
        }
        else {
            length = - Math.floor(section_parts.length / 2);
        }

        current_index += Math.floor(length / 2);
        if (item_distance - section_parts[current_index].distance < tmp_difference){
            tmp_difference = item_distance - section_parts[current_index].distance;
