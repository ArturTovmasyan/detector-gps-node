...
function findNearestSectionPart(item){
    ...
    var line_number = loader.imeis[item.imei];
    //If lines didn't loaded then return error with corresponding message
    if (!line_number) {
        throw new Error(item.imei + " Lines yet didn't loaded. You can try after few minutes");
    }
 ...
 * @param sectionId
 */
function collectBusPlace(imei, sectionId)
{
    ...
    sections[sectionId].route_section.forEach(function (routeSection) {
        ...
        if (routeSectionOrder[imei][routeSection.route.id].indexOf(routeSection.order) == -1) {
            ...
            console.log(routeSectionOrder[imei][routeSection.route.id]);
        }
...
function isApproximatelyRising(arr){
    ...
    return (!isLess || (isGreat/isLess > 2));
}

module.exports.isApproximatelyRising = isApproximatelyRising;

module.exports.routeSectionOrder = routeSectionOrder;

module.exports.collectBusPlace = collectBusPlace;
