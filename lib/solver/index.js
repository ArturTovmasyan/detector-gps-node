...
var determineSecOrdAng = require('./determineSectionOrderAngle').determineSectionOrderAngle;
...
function start() {
    ...
    dataListener.on('data', function (gpsData) {
        ...
        try {
            var sectionOrderAngle = determineSecOrdAng(gpsData.imei, gpsData.route_id, sectionId);
            busInfo.section_order = sectionOrderAngle ? sectionOrderAngle.order : null;
            busInfo.angle         = sectionOrderAngle ? sectionOrderAngle.angle : null;
        }
        catch (e) {
            console.error(process.pid + " PID:_____" + e.message);
        }
        ...
        //Save last updated data of the bus
        BusLastData.update(
            {
                timestamp:    gpsData.timestamp,
                latitude:     gpsData.latitude,
                longitude:    gpsData.longitude,
                line_number:  busInfo.lineNumber,
                plate_number: busInfo.plateNumber
            },
