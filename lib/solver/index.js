...
loader.loadLineSectionParts(47);
...
function findNearestSectionPart(item){
    if(!item.imei || !item.latitude || !item.longitude){
        ...
        return null;//new Error("The item is not valid");
    }

    var line_number = loader.imeis[item.imei];
    //If lines didn't loaded then return error with corresponding message
    if (!line_number) {
        return null;//new Error("Lines yet didn't loaded. You can try after few minutes");
    }
    ...
    if (!section_parts) {
        ...
        return null;//new Error("Section parts yet didn't loaded. You can try after few minutes");
    }
    ...
    return section_parts[dif_index];
}
