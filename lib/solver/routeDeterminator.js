...
function determineRoute(imei, currentSectionId)
{
    ...
    var routeSectionOrder = cache.get('determineRoute_' + imei) ? cache.get('determineRoute_' + imei) : {};
    ...
    if (!currentSectionId){
        if (routeSectionOrder['offLineDataCount'] <= solver_conf.OFF_LINE_DATA_COUNT_TO_CHANGE_STATUS){
            ...
            cache.del('determineRoute_' + imei);
    ...
    for (var key in routes)
    {
        ...
        if (sectionOrderInRoutes[route.id] && routeSectionOrder[imei]
            && ((sectionOrderInRoutes[route.id] >= (route.route_section_max_order - solver_conf.LAST_SECTIONS_COUNT)
            && routeSectionOrder[imei]['onSameSectionCount'] >= solver_conf.MAX_STAY_COUNT_ON_LAST_SECTIONS)
            || routeSectionOrder[imei]['onSameSectionCount'] >= solver_conf.MAX_STAY_COUNT_ON_SECTION))
        {
            cache.del('determineRoute_' + imei);
            cache.del('determineSectionOrder_' + imei);
            return null;
    ...
    if (currentRouteId) {
        ...
        cache.put('determineRoute_' + imei, routeSectionOrder, cache_conf.expiration_time);
