...
function determineRoute(imei, currentSectionId){
    ...
    route_ids.forEach(function (route_id) {
        if (!currentSectionId && routeSectionOrder[imei]) {
            var curDate = new Date();
            if ((curDate - routeSectionOrder[imei][route_id]['lastInserted']) / 1000 <= solver_conf.TIME_TO_CHANGE_STATUS){
                currentRouteId = routeSectionOrder[imei]['lastRouteId'];
...
function collectBusPlaces(imei, currentSectionId)
{
    ...
    sections[currentSectionId].route_section.forEach(function (routeSection) {
        ...
        if (!routeSectionOrder[imei][routeSection.route.id] || !Array.isArray(routeSectionOrder[imei][routeSection.route.id]['orderSequence'])) {
            routeSectionOrder[imei][routeSection.route.id] = {orderSequence: []};
        }

        console.log(imei, routeSectionOrder[imei][routeSection.route.id]['orderSequence'], routeSectionOrder[imei][routeSection.route.id]['lastInserted']);
