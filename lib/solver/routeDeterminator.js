...
var routeSectionOrder = {8888: process.pid};
...
function determineRoute(imei, currentSectionId)
{
    ...
    var sectionOrderInRoutes = collectBusPlaces(imei, currentSectionId);
    ...
    for (var key in routes)
    {
        ...
        if (sectionOrderInRoutes[route.id] && sectionOrderInRoutes[route.id] >= (route.route_section_max_order - solver_conf.LAST_SECTIONS_COUNT) && routeSectionOrder[imei]['onSameSectionCount'] >= solver_conf.MAX_STAY_COUNT_ON_LAST_SECTIONS){
            ...
            if (routeSectionOrder[imei][route.id] && isApproximatelyRising(routeSectionOrder[imei][route.id]['orderSequence'])
                ...
                && (currentDate - routeSectionOrder[imei][route.id]['lastInserted']) / 1000 < solver_conf.OFF_LINE_TIME_TO_CHANGE_STATUS) {
...
function collectBusPlaces(imei, currentSectionId)
{
    ...
    sections[currentSectionId].route_section.forEach(function (routeSection) {
        ...
        //console.log(process.pid, imei, routeSectionOrder[imei][routeSection.route.id]['orderSequence']);
        console.log(process.pid, routeSectionOrder[8888]);
