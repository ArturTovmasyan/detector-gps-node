...
function afterLinesLoad() {
    ...
    sequelize.query("SELECT imei, MAX(end_date) as lastDate " +
        ...
        .then(function (results) {
            ...
            var imeiLastTimestamps = {};
            results = results[0];
            if (results.length){
                lastTimestamp = results[0].lastDate;
                for(var i in results){
                    if (lastTimestamp > results[i].lastDate){
                        lastTimestamp = results[i].lastDate;
                    }

                    imeiLastTimestamps[results[i].imei] = results[i].lastDate;
                }
            }
            else {
            ...
            for (var k in loader.lines){
                afterLineRoutesLoad(loader.lines[k], lastTimestamp, imeiLastTimestamps);
            }
        });
}
 ...
 * @param lastTimestamp
 */
function afterLineRoutesLoad(lineRoutes, lastTimestamp, imeiLastTimestamps)
{
    var routes = [];
    ...
    if (routes.length) {
        ...
        setTimeout(function () {
            ...
            console.log("SELECT * " +
                "FROM gps_data as i " +
                "WHERE TIMESTAMPDIFF(MINUTE, '" + lastTimestamp.toISOString().replace(/T/, ' ').replace(/\..+/, '') + "', i.timestamp) > 0 AND i.route_id IN (" + routes.join(" , ") + ") " +
            ...
            "ORDER BY i.timestamp ")
                .then(function (results) {
                    ...
                    for (key in results) {
                        ...
                        if (imeiLastTimestamps[imei] && results[key].timestamp <= imeiLastTimestamps[imei]){
                            continue;
                        }
                    ...
                    for (var im in busesRoutes) {
                        for (var num in busesRoutes[im]) {
                            if (busesRoutes[im][num][1] > 10 && busesRoutes[im][num][2] > 10 && busesRoutes[im][num][3] > 10 && busesRoutes[im][num][4] > 10
                                && (!busesRoutes[im][num].end_data_before_timestamp || busesRoutes[im][num].end_data_before_timestamp < 10)) {
