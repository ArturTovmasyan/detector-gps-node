        ...
        .then(function (results) {
            ...
            sequelize.query("SELECT imei FROM bus_last_data as bld " +
                ...
                .then(function(imeis) {
                    ...
                    imeis = imeis[0];
                    for(var i in imeis){
                        results.push({imei: imeis[i], lastDate: lastTimestamp});
                    }

                    afterBusLastDataLoad(results[i].imei, results[i].lastDate, 0)
                        .on('ready_for_next_data', function(i) {
                            afterBusLastDataLoad(results[i].imei, results[i].lastDate, i + 1);
                        })
                });
        });
}
...
function afterBusLastDataLoad(imei, lastTimestamp, i)
{
    loader.loadBuses()
        .on(loader_conf.buses_ready_event, function(buses){
            if (!buses.buses[imei] || !buses.buses[imei].lineNumber){
                event.emit('ready_for_next_data', i);
            }

            var lineNumber = loader.buses[imei].line_number;
