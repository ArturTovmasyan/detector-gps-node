    ...
    var netServer = net.createServer(function (socket) {
        ...
        socket.on('data', function (data) {
            ...
            for(var k in param.data_resend_info) {
                ...
                if (!sockets[k]) {
                    sockets[k] = {};
                    sockets[k].socket = new net.Socket();
                    sockets[k].connected = false;
                    sockets[k].socket.number = k;
                    sockets[k].socket.connect(param.data_resend_info[k].port, param.data_resend_info[k].host, function () {
                        var key = this.number;
                        //console.log("Connected to: ", param.data_resend_info[key].port, param.data_resend_info[key].host);

                        if (ipIMEI[socket.key]) {
                            var buf = new Buffer(17);
                            buf.writeUInt16BE(15, 0);
                            buf.write(ipIMEI[socket.key], 2);
                            sockets[key].socket.write(buf);
                            sockets[key].connected = true;
                        }
                    });

                    sockets[k].socket.on('error', function() {
                        var key = this.number;
                        sockets[key] = null;
                        //console.log("Error during connection to: ", param.data_resend_info[key].port, param.data_resend_info[key].host);
                    });
                }

                if (sockets[k] && sockets[k].connected){
                    sockets[k].socket.write(data);
                }
            }
            ...
            if (data.readInt16BE(0) === 15) {
                ...
                if (check.IMEI) {
                    ...
                    console.log('Incoming request from: ' + ipIMEI[socket.key]);
                }
                else {
                    socket.write(check.retData);
                }
        ...
        socket.on('close', function(had_error) {
            console.info('connection closed ip:' + socket.key + ' IMEI:' + ipIMEI[socket.key]);
