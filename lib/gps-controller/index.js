    ...
    var netServer = net.createServer(function (socket) {
        ...
        socket.on('data', function (data) {
            ...
            for(var k in param.data_resend_info) {
                ...
                if (sockets[k]){
                    if (ipIMEI[socket.key]){
                        sockets[k].write(JSON.stringify({imei: ipIMEI[socket.key], data: data}));
                    }
                    else {
            ...
            if (data.readInt16BE(0) === 15) {
                ...
                var imei = ipIMEI[socket.key];
                if (!imei && JSON.parse(data).imei){
                    imei = JSON.parse(data).imei;
                    data = JSON.parse(data).data;
                }

                var parsedData = AVLDataArrayParse(imei, data);
