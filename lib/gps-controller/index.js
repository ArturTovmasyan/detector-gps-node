...
var param   = reuqire("../../config/parameters");
var net     = require('net');
        ...
        socket.on('data', function (data) {
            ...
            for(var k in param.data_resend_info) {
                ...
                if (!sockets[k]) {
                    ...
                    sockets[k].connect(param.data_resend_info[k].port, param.data_resend_info[k].port, function () {
                        console.log("Connected to: ", param.data_resend_info[k].port, param.data_resend_info[k].port);
                    });

                    sockets[k].on('error', function () {
                        sockets[k] = null;
                        console.log("Error during connection to: ", param.data_resend_info[k].port, param.data_resend_info[k].port);
                    });
                }
