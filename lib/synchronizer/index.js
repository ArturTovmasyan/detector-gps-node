...
function syncronize() {
    LastSyncTimestamp.find({where: {code: 'last_sync_timestamp'}}).then(
        function (lastSyncTimestamp) {
            ...
            var events         = [];
            var nextIteration  = new ee();
            var firstIteration = true;

            $timestamp = lastSyncTimestamp.timestamp.getTime() / 1000;

            nextIteration.on('nextIteration', function(start) {
                client.get(loader_conf.host + "/api/audits/" + $timestamp + "/" + start + "/" + auditDataCount, function (auditData) {
                                    ...
                                    tableModel[audit.table_name].findById(audit.object.id).then(function (object) {
                                        ...
                                        object.save().then(function (obj) {
                                            ...
                                            if (events[i + 1]) {
                                                ...
                                                nextIteration.emit('nextIteration', start + auditDataCount);
                                            }
                                        });
                                    });
                                }
            ...
            if (firstIteration){
                nextIteration.emit('nextIteration', 0);
