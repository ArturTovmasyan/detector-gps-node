...
app.post('/api/charts_data', function (req, res) {
    res.pipe(through2(function(data, encoder, next) {

            if (!data.imei) {

                db.getDataGroupedByIMEIs(data.from, data.to, function (err, rows, fields) {
                    if (err) {
                ...
                db.getDataGroupedByDates(data.imei, data.from, data.to, function (err, rows, fields) {
                    if (err) {
                        console.error('error during get data ' + err);
                    }
