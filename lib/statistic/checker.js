...
.then(function(result) {
        ...
        for(var k in routeIds) {
            ...
            loader.getSectionPartsStatistic(routeId, true).then(function() {
                loadedCount++;
                if (loadedCount == 3 * routeIds.length){
                    startCalculate();
                }

                console.log(loadedCount, 3 * routeIds.length);
            });

            loader.getSectionPartsOrders(routeId, true).then(function() {
                loadedCount++;
                if (loadedCount == 3 * routeIds.length){
                    startCalculate();
                }
            ...
            if (routeMinIds[routeId]) {
                sequelize.query("SELECT i1.id, i1.route_id, i2.section_part_id as sp1, i1.section_part_id as sp2, i2.timestamp as t1, i1.timestamp as t2 " +
                    ...
                    .then(function (result) {
                        ...
                        console.log('will be: ', stat.passTime, ' be: ', result[0].t2 - result[0].t1);
