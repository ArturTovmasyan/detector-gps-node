...
sequelize.query("SELECT i.route_id, i.timestamp, i.section_part_id, ssp.stop_id FROM gps_data AS i JOIN section_part as sp ON sp.id = i.section_part_id LEFT JOIN stop_section_part as ssp ON ssp.section_part_id = sp.id WHERE i.imei = " + imei + " AND date(i.timestamp) = '" + date + "'",
        ...
        .then(function(result) {
                ...
                setTimeout(function() {
                    ...
                    for (var k = 0; k < result.length; k++){
                        ...
                        if (item.stop_id){
                            try {
                                var stat = statistic.getTimeStatistics(item.route_id, firstItem.section_part_id, item.section_part_id);
