...
function CalculateLineForecastingErrors(lineNumber, date, logging) {
                            ...
                            sequelize.query("SELECT i.imei, i.route_id, i.timestamp, i.section_part_id, ssp.stop_id " +
                                ...
                                .then(function (result) {
                                    ...
                                    if (busCount == Object.keys(lineBuses).length){
                                        event.emit('ForecastingError', {lineNumber: lineNumber, date: date, alg1: allSum / allCount, alg2: allSumCoeff / allCount});
                                        if (logging) {
...
function CalculateAllForecastingErrors()
{
    try {
        loader.getStatisticMode();
    }
    ...
    rawLoader.loadLines().on(loader_conf.lines_ready_event, function(data){
        ...
        for(var line in lines){
            var date = new Date();
            CalculateLineForecastingErrors(line, date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate())
                .on('ForecastingError', function(data){
