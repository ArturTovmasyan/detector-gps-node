...
function getTimeStatistics(routeId, sectionPartId1, sectionPartId2) {
    ...
    for (var i = sectionPartOrder1; i <= sectionPartOrder2; i++){
        ...
        if (sectionPartStatistic[i].stop_id){
            stopTimes[sectionPartStatistic[i].stop_id] = sectionPartStatistic[i].pass_count - sectionPartStatistic[sectionPartOrder1].pass_count;
        }
    }

    return {passTime: sectionPartStatistic[sectionPartOrder2].pass_count - sectionPartStatistic[sectionPartOrder1].pass_count, stopTimes: stopTimes};
}
...
setInterval(function() {
    try {
        console.log(getTimeStatistics(9, 29925, 28460));
    }
    catch (e) {
