 ...
 *
 * @param routeId
 * @param dontGenerateError
 * @returns {*}
 */
function getSectionPartsOrders(routeId, dontGenerateError) {
    ...
    if (dontGenerateError) {
        return section_parts_order_event[routeId];
    }
 ...
 * @returns {*}
 */
function getSectionPartsStatistic(routeId, dontGenerateError) {
    ...
    if (!section_parts_statistic_event[routeId] || !section_parts_statistic_event[routeId][statisticModeCode]) {
        ...
        section_parts_statistic_event[routeId][statisticModeCode] =
            sequelize.query("set @order=0;").then(function(result) {
                sequelize.query("set @order1=0;").then(function(result) {
                    sequelize.query("select t1.num, t1.stop_id, " +
                        ...
                        , {type: sequelize.QueryTypes.SELECT})
                    .then(function (results) {
                        if (delete sectionPartsStatistic[routeId]) {
                            delete sectionPartsStatistic[routeId];
                        }
                        ...
                        .then(function(result) {
                            ...
                            setTimeout(function() {
                                console.log(process.pid + '___ Reload statistic ....');
                                sectionPartsStatistic[routeId]         = {};
                                section_parts_statistic_event[routeId] = {};
                            }, intervalToLoad);
                        });
    ...
    if (dontGenerateError) {
        return section_parts_statistic_event[routeId][statisticModeCode];
    }
...
from
    (select @order:=@order+1 as num, t.id, t.stop_id, t.pass_time
     from
     (select SUM(stat.pass_times_sum / stat.pass_count) as pass_time
     from section as s
     join section_part as sp on sp.section_id = s.id
     join route_section as rs on rs.section_id = s.id
