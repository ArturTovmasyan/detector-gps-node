...
function getSectionPartsStatistic(routeId) {
    ...
    if (!section_parts_statistic_event[routeId] || !section_parts_statistic_event[routeId][statisticModeCode] && queriesCount == 0) {

        queriesCount++;
        section_parts_statistic_event[routeId] = {};


        sequelize.query("set @order" + routeId + "=0;").then(function(result) {});
        sequelize.query("set @order1" + routeId + "=0;").then(function(result) {});
        section_parts_statistic_event[routeId][statisticModeCode] =
            sequelize.query("select t1.num, t1.stop_id, " +
                                    " (select SUM(t2.pass_time) " +
                                            "from " +
                                                "(select @order1" + routeId + ":=@order1" + routeId + "+1 as num, t.pass_time " +
                                                "from " +
                ...
                "from " +
                    "(select @order" + routeId + ":=@order" + routeId + "+1 as num, t.id, t.stop_id, t.pass_time " +
                    "from " +
                        "(select sp.id, st.stop_id as stop_id, (stat.pass_times_sum / stat.pass_count) as pass_time " +
                        "from section as s " +
                        "join section_part as sp on sp.section_id = s.id " +
            ...
            .then(function (results) {
                ...
                console.log('statisticModeCode: ', statisticModeCode);
