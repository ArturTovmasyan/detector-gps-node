            ...
            sequelize.query("set @order=0;").then(function(result) {
                sequelize.query("set @order1=0;").then(function(result) {
                    ...
                    .then(function (results) {
                        ...
                        sequelize.query("SELECT MIN(pass_count) as min_cnt FROM statistic WHERE route_id = " + 2 + " and statistic_mode_code = '" + 0 + "'",
                            {type: sequelize.QueryTypes.SELECT})
                        .then(function(result) {
                            var intervalToLoad = 0;
                            if (result[0].min_cnt < 100){
                                intervalToLoad = 60000;
                            ...
                            setTimeout(function() {
                                console.error('Reload statistic ....');
                                sectionPartsStatistic[routeId]         = {};
                                section_parts_statistic_event[routeId] = {};
                            }, intervalToLoad);
                        });
...
module.exports.commands = {
    'syncronize': {
        "run": reload,
        description: "This command is used to reload all data"
    }
};
